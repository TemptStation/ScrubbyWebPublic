    ...
    public class SqlRoundService : SqlServiceBase, IRoundService
    {
        ...
        private async Task<ScrubbyRound> GetRoundInternal(int roundId)
        {
            ...
            const string query = @"
                ...
                WHERE
                    r.id = @roundId
                    AND rf.name = 'runtime.txt'";

            await conn.QueryAsync<ScrubbyRound, RoundBuildInfo, TestMerge, ScrubbyRound>(query, (round, version, testmerge) =>
            {
                toReturn ??= round;
                if (version == null) 
                    return round;
        ...
        public async Task<int> GetNext(int id, bool forward = true, List<string> ckey = null)
        {
            ...
            if (ckey != null && ckey.Any())
                query.AppendLine("SELECT id FROM get_rounds_for_players(@ckey, false)");
            else
