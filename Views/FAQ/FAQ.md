...
##### Gaining Access
To gain access to the API is simple, all you need to do is [log into Scrubby](https://scrubby.melonmesa.com/login) using your tgstation account, and you will automatically be issued one API key. This API key can be accessed on your [user page](https://scrubby.melonmesa.com/me) which can be accessed by clicking on your name in the top-right of your browser.

By mousing over an API key on your user page, you can see various limits and restrictions on that key. They will be enforced by the server if you attempt to exceed them, though the server will respond with a verbose response as to why.

##### Using API Keys
To make use of an API key to access Scrubby data, send a POST request to an [API endpoint](#api-endpoint-documentation) with the the header key `X-Scrubby-API-Key`'s value set to the value of your API key. The body format of individual API endpoints are described below.

##### A Note On Abuse
Attempts to abuse the API endpoints provided by Scrubby will be met with, at best, an attempt to reach out from bobbahbrown#0001 and at worst a site-wide ban from Scrubby. Please do not attempt to abuse this.

##### Oh God Please Help It Isn't Working!
If you are experiencing issues with the API, please send a message to the /tg/station discord mentioning bobbahbrown#0001, or DM me directly on discord.

### API Endpoint Documentation

##### Log Data Aggregation
Use this API endpoint for aggregating collections of log messages (individual lines) from Scrubby's database. 

Endpoint: `https://scrubby.melonmesa.com/api/da/aggregate`  
Request Type: `POST, JSON body`  

The fields for this API endpoint are described below. Fields that are italicized are optional, and can be excluded from the API call.
  
|       Key       	|    Type    	|                                                            Description                                                            	|
|:---------------	|:----------	|:---------------------------------------------------------------------------------------------------------------------------------	|
| UpperRoundLimit 	| int        	| Defines the upper limit of the rounds to aggregate data from                                                                      	|
| LowerRoundLimit 	| int        	| Defines the lower limit of the rounds to aggregate data from                                                                      	|
| TypeFilters     	| *string[]* 	| Defines message types to include (like "SAY", "ADMIN", "OOC"...)                                                                  	|
| ContentFilter   	| *regex*    	| Defines a regular expression to evaluate on each line (Must be regex formatted, like /test/i)                                     	|
| Files           	| string[]   	| Defines the list of files to retrieve messages from (like "game.txt")                                                             	|
| ResponseLimit   	| *int*      	| Defines the limit of documents to retrieve                                                                                        	|
| GroupByRound    	| *bool*     	| Defines if the returned data should be grouped by rounds                                                                          	|
| NoMetadata      	| *bool*     	| Defines if the returned data should include any fields other than the Message field (like location, message type, timestamp, etc) 	|

The response format is a document like the following...
    ...
    "data": [
        {
            ...
            "messages": [
                {
                    "MessageID": "5ca6890d1d256f2bd07bff85",
                    "Timestamp": "2019-02-01T16:36:25.809Z",
                    "Type": "ADMIN",
                    "Message": "Gigapuddi420/(Katty Guillou) checked the individual player panel for Bobbahbrown/(Kashindra Kalcronk).",
                    "Origin": null,
                    "OriginName": null,
                    "RelativeIndex": 1628,
                    "ParentFile": "5ca647255585710334f3341e",
                    "FailedParse": false
                }
            ]
        }
